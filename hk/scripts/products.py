        ...
        for mp_image in mongo_product['assets']:
            ...
            if p_image_from_log:
                ...
                if mongo_asset:
                    ...
                    invalid_asset_to_log = InvalidAssets(mongo_id=mp_image, parent=mongo_product['_id'], category='product')
                    invalid_asset_to_log.save()
    if mp_assets:
    ...
    if woo_ins_data:
        if 'id' not in woo_ins_data:
            ...
            for mp_image in mongo_product['assets']:
                p_image_from_log = get_image_from_log(mongo_id=mp_image, category='product')
                if p_image_from_log:
                    mp_assets_retry.append({'id': p_image_from_log.woo_id})
                else:
                    mongo_asset = mapi['assets'].find_one({'_id': mp_image})
                    # check if image exist on url
                    if mongo_asset:
                        ...
                        image_formats = ("image/png", "image/jpeg", "image/jpg")
                        if response.headers['content-type'] in image_formats:
                            mp_assets_retry.append({
                                'src':  mongo_asset['url'],
                                'name': mongo_asset['_id']
                            })
                        else:
                            print('invalid image', response.headers['content-type'])
                            invalid_asset_to_log = InvalidAssets(mongo_id=mongo_asset['_id'], parent=mongo_product['_id'], category='product')
                            invalid_asset_to_log.save()
                    else:
                        invalid_asset_to_log = InvalidAssets(mongo_id=mongo_asset['_id'], parent=mongo_product['_id'], category='product')
                        invalid_asset_to_log.save()
            if mp_assets_retry:
                product_data['images'] = mp_assets_retry
            else:
                product_data['images'] = []
            woo_ins_data = woo_product_insert(wapi, product_data)
...
def update_product(mapi, wapi, woo_id, mongo_product, cc_rate):
    oldPrice = 0
    price = 0
    rate = 1
    if 'currency' in mongo_product and mongo_product['currency'] != 'USD' and mongo_product['currency'].lower() in cc_rate:
        rate = cc_rate[mongo_product['currency'].lower()]['rate']
    if 'oldPrice' in mongo_product and mongo_product['oldPrice'] not in ['', None]:
        oldPrice = float(mongo_product['oldPrice']) / 100
        oldPrice = oldPrice / rate
    ...
    woo_ins_data = woo_product_update(wapi, woo_id, product_data)
